...
void EdgeBasedGraphFactory::Run(const char * originalEdgeDataFilename) {
	...
	std::vector<NodeID> deletedNodes1/*, deletedNodes2*/;
	std::vector<NodeID> in_degrees_vector(_nodeBasedGraph->GetNumberOfNodes(), 0);


	/** count the in-degree for each node **/
	for(_NodeBasedDynamicGraph::NodeIterator u = 0; u < _nodeBasedGraph->GetNumberOfNodes(); ++u ) {
		//in_degrees_vector[u] += _nodeBasedGraph->GetOutDegree(u);
		for(_NodeBasedDynamicGraph::EdgeIterator e1 = _nodeBasedGraph->BeginEdges(u); e1 < _nodeBasedGraph->EndEdges(u); ++e1) {
			const _NodeBasedDynamicGraph::NodeIterator v = _nodeBasedGraph->GetTarget(e1);
			++in_degrees_vector[v];
		}
	}
	//
	...
	for(_NodeBasedDynamicGraph::NodeIterator u = 0; u < _nodeBasedGraph->GetNumberOfNodes(); ++u ) {
		for(_NodeBasedDynamicGraph::EdgeIterator e1 = _nodeBasedGraph->BeginEdges(u); e1 < _nodeBasedGraph->EndEdges(u); ++e1) {
			...
			for(_NodeBasedDynamicGraph::EdgeIterator e2 = _nodeBasedGraph->BeginEdges(v); e2 < _nodeBasedGraph->EndEdges(v); ++e2) {
				...
				bool node_is_degree_two_in_oneways = ((u != w) && (1 == in_degrees_vector[v]) && (1 == _nodeBasedGraph->GetOutDegree(v)) );
				bool node_is_degree_four_in_twoways = ((u != w) && (2 == in_degrees_vector[v] && 2 == _nodeBasedGraph->GetOutDegree(v)) && (edgeData1.forward && edgeData1.backward) && (edgeData2.forward && edgeData2.backward));
	...
	for(_NodeBasedDynamicGraph::NodeIterator u = 0; u < _nodeBasedGraph->GetNumberOfNodes(); ++u ) {
		for(_NodeBasedDynamicGraph::EdgeIterator edgeID = _nodeBasedGraph->BeginEdges(u); edgeID < _nodeBasedGraph->EndEdges(u); ++edgeID) {
			...
			for(_NodeBasedDynamicGraph::EdgeIterator e2 = _nodeBasedGraph->BeginEdges(v); e2 < _nodeBasedGraph->EndEdges(v); ++e2) {
				...
				if( !isBollardNode ) { //only add an edge if turn is not a U-turn except it is the end of dead-end street.
					if (!CheckIfTurnIsRestricted(u, v, w) || (onlyToNode != UINT_MAX && w == onlyToNode)) { //only add an edge if turn is not prohibited
						...
						if(edgeData2.edgeBasedNodeID >= _nodeBasedGraph->GetNumberOfEdges())
							INFO("edgeData2.edgeBasedNodeID: " << edgeData2.edgeBasedNodeID << ", _nodeBasedGraph->GetNumberOfEdges(): " << _nodeBasedGraph->GetNumberOfEdges());
